    ...
    <ul id="lista-tareas">
       @foreach ($tareas as $tarea)
        ...
        <li @if ($tarea->realizada) == 1) style="text-decoration:line-through;" @endif>
          ...
          {{ $tarea->tarea}} <span class="badge" style="background:{{$tarea->categoria->color}}">{{ $tarea->categoria->nombre}}</span>  <a href="#" title="Eliminar tarea" class="btn btn-circulo btn-danger">X</a></li>
        @endforeach
    </ul>
  </div>
</div>
